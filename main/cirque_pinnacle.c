...
static const char *TAG = "pinnacle-touchpad";
...
//Functions for timer.h port to ESP32
uint16_t timer_read(void){
    uint16_t current_time = (esp_timer_get_time() / 1000);
    return current_time;
}

uint16_t timer_elapsed(uint16_t timeout_timer){
    uint16_t current_time = (esp_timer_get_time() / 1000);
    return (current_time - timeout_timer);
}
...
void cirque_pinnacle_enable_feed(bool feedEnable) {
    ...
    ESP_LOGI(TAG, "disblefeed = %d", feedEnable);
...
ESP_LOGI(TAG, "touchpad init done %d", touchpad_init);
}
...
pinnacle_data_t cirque_pinnacle_read_data(void) {
    ...
    ESP_LOGI(TAG, "%s: xLo=%3d xHi=%3d yLo=%3d yHi=%3d\n", __FUNCTION__, xMin, xMax, yMin, yMax);
#endif
